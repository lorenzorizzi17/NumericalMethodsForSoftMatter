...
In the previous sections, I integrated the equations of motion for the $N$-particles system with the help of LAMMPS and focused on the offline analyses of the relevant macroscopic quantities (kinetic and potential energy, radial distribution functions, \dots). Here, I decided to modify the code written for the MC simulation of Chapter 1 hence implementing myself a modest Velocity-verlet molecular engine. This was done with the prospect of creating a custom thermostat algorithm over which I could exert more control. In fact, a general MD simulation where the EOMs are integrated numerically through a symplectic algorithm will replicate the behavior of a closed system with no external environment acting upon it. Clearly enough, this implies that the correct statistical ensamble to use when describing this system is the microcanonical (or NVE) ensamble, since energy is conserved by Hamilton's theorem.
...
\begin{figure}
	\centering
	\includegraphics[scale = 0.5]{../MolecularDynamics/Ex3/fig/energy_conservation.pdf}
	\caption{\textit{WIP}}
	\label{fig:ThermoNo}
\end{figure}
It's easy to notice a somewhat strange oscillatory trend, the same we observed in Fig.\ref{fig:energyHO}. However, the energy drift with respect to the initial value is on the order of $0.1\%$, a well acceptable value that is also in agreement with LAMMPS simulation (Fig.\ref{fig:energyLJ}).

Next, we can proceed with implementing both Berendsen and Andersen thermostat. 
\subsubsection{Berendsen thermostat}
When using Berendsen thermostat, one can control the intensity of the coupling with the system via the parameter $\tau$ (measured in $\delta t$ units). In Fig.\ref{fig:kinetic_berendsen} we illustrate the temperature $T_K$ histograms varying $\tau$ fixing the target temperature $T = 1$ (in adimensional units). 
...
\begin{figure}
	\centering
	\includegraphics[scale = 0.55]{../MolecularDynamics/Ex3/fig/berendsen_kinetic.pdf}
	\caption{\textit{total of 10000 steps, delta T = 0.005, N = 200}}.
	\label{fig:kinetic_berendsen}
\end{figure}

In Fig.\ref{fig:kinetic_berendsen} we've also reported the behavior of the standard deviation $\sigma_T$ as a function of $\tau$. In the canonical ensemble, we'd expect:
$$
...
\begin{figure}
	\centering
	\includegraphics[scale = 0.55]{../MolecularDynamics/Ex3/fig/andersen_size.pdf}
	\caption{\textit{ded}}
	\label{fig:andersenN}
\end{figure}
The same plot concerning Berendsen algorithm is reproduced in Fig.\ref{fig:berendsenN} when $\tau = 10$ or $\tau = 100$. The situation here is quite different and the scaling behavior for $\sigma_T$ is definitely not obeyed by Berendsen thermostat, providing hint of its non-canonical properties.
\begin{figure}
	\centering
	\includegraphics[scale = 0.55]{../MolecularDynamics/Ex3/fig/berendsen_size.pdf}
	\caption{\textit{ded}}
...
\begin{figure}
	\centering
	\includegraphics[scale =0.66]{../MolecularDynamics/Ex3/fig/RDF_plot_crystalline.pdf}
	\caption{\textit{ecwd}}
	\label{fig:rdf}
\end{figure}
...
The equipartition theorem states that, at thermal equilibrium, each quadratic degrees of freedom appearing in the Hamiltonian of the system has an average energy associated of $\frac{1}{2}k_b T = \frac{1}{2}$. 
...
\begin{figure}
	\centering
	\includegraphics[scale =0.5]{../MolecularDynamics/Ex3/fig/equipartition.pdf}
	\caption{\textit{ecwd}}
