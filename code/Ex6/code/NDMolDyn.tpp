...
void NDMolDyn<Dim>::buildVerletLists(){
    ...
    // First step, build the cell list (assuming a cubic box divided in M cells per dimension)
    ...
    for(auto it = m_particles.begin(); it != m_particles.end(); ++it){
        ...
        for(int k = 0; k < Dim; ++k){
            ...
            //assert(idx_k < m_M); // !! assert qui
            cell_idx[k] = idx_k; 
        } // Now I have the multiindex
        int linear_idx = 0;
        for(int k = 0; k < Dim; ++k){
            linear_idx += cell_idx[k] * std::pow(m_M, k); //row-major
        }
    ...
    // Now the cell list is built. Next step, build the verlet lists
    //First, loop over all cells
    for(int cell_idx = 0; cell_idx < powerDim<int, Dim>(m_M); ++cell_idx){
        // For each cell, loop over all particles in that cell. Start with headOfChain
        ...
        while(particle_idx != -1){
            ...
            for(int k = 0; k < Dim; ++k) { // Get the multi-index of the current cell
                current_cell_coords[k] = temp_idx % m_M;
                temp_idx /= m_M;
            }
            // Iterate over neighboring cells (3^Dim)
            for(int n = 0; n < powerDim<int, Dim>(3); ++n) {
                int neighbor_cell_linear_idx = 0;
                int stride = 1; // 1, M, M^2...
                int temp_n = n; // Copia per decodificare la base 3
                for(int k = 0; k < Dim; ++k) {
                    // Decodifica offset: (0, 1, 2) -> (-1, 0, 1)
                ...
                // Now neighbor_cell_linear_idx is the linear index of the neighbot cell
                // Get the index of a particle in that cell
                int j_idx = m_headOfChain[neighbor_cell_linear_idx];                
                while(j_idx != -1) {
                    if(particle_idx != j_idx) { // No self-interaction
                        
                        vecd pos_j = m_particles[j_idx].position;
                        // Compute finally the squared distance between pos_i and pos_j with MIC
                        double d2 = dist_sq_pbc(pos_i, pos_j);
                        if(d2 < threshold_sq) {
                            verletList.push_back(j_idx);
                        }
                    }
                    // Get to the next particle in that cell
    ...
    // Now the verlet lists should have been correctly built
    // Finally, save a copy of the particles positions in backupParticles
...
Patch perturbPatch(Patch const& old_patch, double moduler) {
...
void NDMolDyn<Dim>::singleMCStep(){ //Run a single step of MC (thus N proposals)
    ...
    bool move_position = (static_cast<double>(rand()) / RAND_MAX) < 0.5;
    ...
    if(move_position){
        ...
        Patch new_patch = perturbPatch(m_particles[particle_idx].getPatch(), m_gamma);
        // Compute the new energy
