...
(Part1)
1) Sampling **random numbers** from a given arbitrary 1D distribution (using the inverse CDF or rejection method) or from a multi-dimensional distribution.
...
(Part 2)
6) Simulation of the **(Patchy) Hard Sphere model** implementing Cell Lists and **Verlet Lists** for efficient neighbor searching in MD
   
7) **Equations of motion numerical integrators**: 1st order Euler, symplectic integrators like Velocity Verlet. LAMMPS software as an efficient MD simulator.
   
8) Molecular dynamics simulation in the **canonical ensemble**; algorithmic implementation of canonical and non-canonical **thermostats** (Berendsen, Andersen)

9) **Multiple Markov chains and Parallel Tempering.** Multiple Histograms method to extract thermal averages at an arbitrary $\beta$
    
10) **Gillespie** algorithm for continuous time Markov processes

11) Numerical implementation of **Langevin and Brownian processes.**
